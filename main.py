...
MAX_SERVER_AGE = 172800          # Remove after 48 Hours
...
app = Flask('')
@app.route('/')
def home(): return "✅ TRACKER ONLINE — READY"
...
class RobloxAutoTracker(commands.Bot):
    ...
    # 🕵️‍♂️ SCAN ALL SERVERS — EXACT SAME AS ROVALRA
    ...
    def _get_real_start_time(self, job_id, server_data):
        try:
            # Extract timestamp directly from JobID (Roblox internal format)
            if "T" in job_id and "Z" in job_id:
                dt_str = job_id.split("_")[0]
                return datetime.strptime(dt_str, "%Y%m%dT%H%M%SZ")
        except: pass
        # Fallback: Calculate from current playing time data
    ...
    def process_spawn_logic(self, jid, start_time, now):
        ...
        boss_number = int(age // BOSS_CYCLE) + 1
        boss_total = int(MAX_SERVER_AGE // BOSS_CYCLE)

        # --- RIFT LOGIC (1h30m Cycle) ---
        until_rift = RIFT_CYCLE - (age % RIFT_CYCLE)
        rift_number = int(age // RIFT_CYCLE) + 1
        rift_total = int(MAX_SERVER_AGE // RIFT_CYCLE)

        # 🟠 BOSS: 5 MINUTE WARNING
        if WARN_BEFORE - 10 < until_boss <= WARN_BEFORE:
            embed = discord.Embed(
                title="🚨 BOSS SPAWN WARNING",
                description=(
                    f"**Boss #{boss_number}/{boss_total}** is spawning soon!\n\n"
            ...
            embed.set_footer(text="Roblox Auto-Tracker • Powered by RoValra Logic")
            self.send_embed(BOSS_WEBHOOK, embed, jid, f"boss_warn_{boss_number}")
        ...
        if 0 <= until_boss <= 15:
            embed = discord.Embed(
                ...
                description=(
                    f"**Boss #{boss_number}/{boss_total}** has appeared!\n\n"
                    f"⏱️ **Server Uptime:** `{self._fmt_time(age)}`\n"
            ...
            embed.set_footer(text="Roblox Auto-Tracker • Powered by RoValra Logic")
            self.send_embed(BOSS_WEBHOOK, embed, jid, f"boss_now_{boss_number}")

        # 🔵 RIFT: 5 MINUTE WARNING
        if WARN_BEFORE - 10 < until_rift <= WARN_BEFORE:
            embed = discord.Embed(
                title="🌀 RIFT SPAWN WARNING",
                description=(
                    f"**Rift #{rift_number}/{rift_total}** is opening soon!\n\n"
                    f"⏱️ **Server Uptime:** `{self._fmt_time(age)}`\n"
            ...
            embed.set_footer(text="Roblox Auto-Tracker • Powered by RoValra Logic")
            self.send_embed(RIFT_WEBHOOK, embed, jid, f"rift_warn_{rift_number}")
        ...
        if 0 <= until_rift <= 15:
            embed = discord.Embed(
                ...
                description=(
                    f"**Rift #{rift_number}/{rift_total}** is active!\n\n"
                    f"⏱️ **Server Uptime:** `{self._fmt_time(age)}`\n"
                    f"🔗 **Join Fast:** [Click Here]({link})"
                ),
            ...
            embed.set_footer(text="Roblox Auto-Tracker • Powered by RoValra Logic")
            self.send_embed(RIFT_WEBHOOK, embed, jid, f"rift_now_{rift_number}")
